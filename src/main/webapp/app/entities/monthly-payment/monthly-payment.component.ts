...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveMonthlyPayments = async () => {
      ...
      try {
        ...
        const res = await monthlyPaymentService().retrieve(paginationQuery);
    ...
    onMounted(async () => {
      await retrieveMonthlyPayments();
    });

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IMonthlyPayment) => {
    ...
    const removeMonthlyPayment = async () => {
      try {
        await monthlyPaymentService().delete(removeId.value);
        const message = 'A MonthlyPayment is deleted with identifier ' + removeId.value;
